...
class CAB(nn.Module):
    def __init__(self, num_feat, time_channels, compress_ratio=3, squeeze_factor=30):
        super(CAB, self).__init__()
        self.time_emb_proj = nn.Linear(time_channels, num_feat, bias=False)
        self.cab = nn.Sequential(
            nn.Conv2d(num_feat * 2, num_feat // compress_ratio, 3, 1, 1),
            nn.GELU(),
...
class VSSBlock(nn.Module):
    def __init__(
            ...
            d_state: int=16,
            expand: float=2,
            time_channels: int=48.,  # 确保这里添加了 time_channels 参数
            **kwargs,
    ):
        super().__init__()
        ...
        self.conv_blk = CAB(hidden_dim, time_channels)  # 传递 time_channels 给 CAB
        self.ln_2 = nn.LayerNorm(hidden_dim)
        self.skip_scale2 = nn.Parameter(torch.ones(hidden_dim))

    def forward(self, input, t, x_size):
        # x [B,HW,C]
        B, L, C = input.shape
        input = input.view(B, *x_size, C).contiguous()  # [B,H,W,C]
        x = self.ln_1(input)
        x = input*self.skip_scale + self.drop_path(self.self_attention(x))
        x = x * self.skip_scale2 + self.conv_blk(self.ln_2(x).permute(0, 3, 1, 2).contiguous(), t).permute(0, 2, 3, 1).contiguous()
...
class MambaIRUNet(nn.Module):
    ...
    def forward(self, inp_img, t):
        ...
        for layer in self.decoder_level3:
            out_dec_level3 = layer(out_dec_level3, t, [H // 4, W // 4])
        ...
        for layer in self.decoder_level2:
            out_dec_level2 = layer(out_dec_level2, t, [H // 2, W // 2])
        ...
        for layer in self.decoder_level1:
            out_dec_level1 = layer(out_dec_level1, t, [H, W])
...
# if __name__ == '__main__':
    ...
    # # 模拟输入数据和时间编码
    # hidden_dim = 48
    # time_channels = 48*2 ** 3
    # inp_img = torch.randn(1, 3, 256, 256).to(device)
    # diffusion = Diffusion(noise_steps=100, img_size=256, device=device)
    # t = diffusion.sample_timesteps(inp_img .shape[0]).to(device)
    # print(f"Output shape: {t.shape}")
    # time_emb = TimeEmbedding(time_channels).to(device)
    # t = time_emb(t)
    # print(f"Output shape: {t.shape}")

    # # # # # 创建VSSBlock实例并验证
    # # input = torch.randn(1, 32*32, 64).to(device)
    # # vss_block = VSSBlock(hidden_dim=hidden_dim, time_channels=time_channels).cuda()
    # # output = vss_block(input, t, x_size)

    # patch_embed = OverlapPatchEmbed(inp_channels, dim).to(device) 
    # _, _, H, W = inp_img.shape
    # inp_enc_level1 = patch_embed(inp_img)  # b,hw,c
    # out_enc_level1 = inp_enc_level1
    # print(f"Output shape: {out_enc_level1.shape}")
    # num_blocks=[4, 6, 6, 8]
    # mlp_ratio=2.
    # drop_path_rate=0.
    # base_d_state = 4
    # time_channels=48*2 ** 3
    # # # time_emb = TimeEmbedding(time_channels)
    # # # t = time_emb(t)
    # # print(f"Output shape: {t.shape}")

    # encoder_level1 = nn.ModuleList([
    # VSSBlock(
    #     hidden_dim=dim,
    #     drop_path=drop_path_rate,
    #     norm_layer=nn.LayerNorm,
    ...
    # ).to(device) 
    # for i in range(num_blocks[0])])
    # down1_2 = Downsample(dim).to(device)   ## From Level 1 to Level 2
    # encoder_level2 = nn.ModuleList([
    #         VSSBlock(
    #             hidden_dim=int(dim * 2 ** 1),
    #             drop_path=drop_path_rate,
    #             norm_layer=nn.LayerNorm,
    ...
    # for layer in encoder_level1:
    #     out_enc_level1 = layer(out_enc_level1, t, [H, W])
    # print(f"Output shape: {out_enc_level1.shape}")

    # inp_enc_level2 = down1_2(out_enc_level1, H, W)  # b, hw//4, 2c
    # out_enc_level2 = inp_enc_level2
    # print(f"Output shape: {out_enc_level2.shape}")
    # for layer in encoder_level2:
    #     out_enc_level2 = layer(out_enc_level2, t, [H // 2, W // 2])
    # print(f"Output shape: {out_enc_level2.shape}")
    # 创建MambaIRUNet实例
    # model = MambaIRUNet(
    ...
    # # 将输入图像和时间步传递给MambaIRUNet
    # output = model(inp_img, t)
