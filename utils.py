...
def ct_parallel_project_2d(img, theta):
    ...
    # Generate meshgrid and move to the same device as img
    y, x = torch.meshgrid([torch.arange(h, dtype=torch.float32, device=img.device) / h - 0.5,
                           torch.arange(w, dtype=torch.float32, device=img.device) / w - 0.5])
    ...
    sample_coords = torch.stack([y_rot, x_rot], dim=0)  # [2, h, w]
