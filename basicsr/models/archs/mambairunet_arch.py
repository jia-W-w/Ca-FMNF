...
class VSSBlock(nn.Module):
    def __init__(
        ...
        self.act1 = Swish()
        self.conv1 = nn.Sequential(nn.Conv2d(hidden_dim, hidden_dim, kernel_size=(3, 3), padding=(1, 1)),
                                   # nn.Dropout(0.01)
                                   )
        self.norm2 = nn.LayerNorm(hidden_dim)
        self.act2 = Swish()
        self.conv2 = nn.Sequential(nn.Conv2d(hidden_dim, hidden_dim, kernel_size=(3, 3), padding=(1, 1)),
                                   # nn.Dropout(0.01)
                                   )
    ...
    def forward(self, input, x_size):
        ...
        B, L, C = input.shape
        input = input.view(B, *x_size, C).contiguous()  # [B,H,W,C]
        # print('ly:', input.shape)
        x = self.conv1(self.act1(self.norm1(input).permute(0, 3, 1, 2).contiguous())).permute(0, 2, 3, 1).contiguous()
        # print('ly:', x.shape)
        # print('ly:', x.shape)
...
if __name__ == '__main__':
    ...
    time_channels = 128
    x_size = (32, 32)
    input = torch.randn(1, 32*32, 48).cuda()
    t = torch.randn(1, time_channels).cuda()

    # 创建VSSBlock实例并验证
    vss_block = VSSBlock(hidden_dim=hidden_dim, time_channels=time_channels).cuda()
    output = vss_block(input, x_size)
